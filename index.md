...
<a id="road-segmentation-reg"></a> Predicting the road segmentation is important in real-time applications, as it projects a future planning space for the autonomous vehicle to plan and control. Supervised models are often used in real-time road detection, so we used the road maps generated by K-Means clustering to train a ridge regression model.

#### Image Pre-processing
Images for the ridge regression model were pre-processed with the following methods:
...
#### Ridge Regression Using Road Maps, Alone
<a id="road-segmentation-ridge-1"></a> First, we tried training a ridge regression model using just the pre-processed images as inputs, with the road maps obtained with unsupervised learning as labels. However, the predictions were generally poor.
...
#### Ridge Regression Using Road Maps + Car Dynamics Data
<a id="road-segmentation-ridge-2"></a> Next, we tried including the AutoRally robot's state and control data (longitudinal velocity, lateral velocity, yaw rate, yaw position, global x position, and global y position, steering angle, wheel speed) in the ridge regression model. Our intuition was that the state and control data should have a strong correlation with the road map because that data describes the car's position in its local environment. The state and control are were concatenated together with a pre-processed camera image to formulate the input to the regression model, and the K-Means road labels obtained earlier were used as ground truth labels.
...
<div style="display: flex; flex-direction: row; justify-content: space-evenly; width: 100%;"><img src="images/predicted_with_dynamics3.png" width="80%"></div>
...
* Jacob Knaup
    * [Neural Network Turn Classifier](https://github.com/mlsu2020/turn_classification)
* Jaein Lim
    * [Supervised learning of road maps using Ridge Regression](https://github.com/mlsu2020/road-prediction)
* Phong Tran
    * [Unsupervised learning of road maps using DBSCAN and K-Means](https://github.com/mlsu2020/road-detection)
    * Video editing
